...
export function ChatHistoryList({ sessions, onSelect, onDelete, onNewSession }: ChatHistoryListProps) {
  return (
    <div className="px-4 py-4 space-y-3">
      {sessions.length === 0 && (
        <p className="text-center text-sm text-washi-dim py-8">{UI_TEXT.plan.noSessions}</p>
      )}

      {sessions.map((session) => {
        const lastMessage = session.messages[session.messages.length - 1];
        return (
          <div key={session.id} className="rounded-lg border border-border bg-soil-surface p-3">
