...
### 15-B S2: スキーマ拡張 + データ反映 — 完了 (2026-04-18)

- [x] `Mushroom.traits?: string[]` を追加 (src/types/mushroom.ts)
- [x] `scripts/phase15/apply_traits.mjs` 新設、species-traits-visible.json から mushrooms.json に埋め込み
- [x] 112 種に traits 付与、1 種 (Entoloma sarcopus) は Trait Circus 未登録のため undefined
- [x] 合計 5,994 trait keys、平均 53.5 / 種
- [x] mushrooms.json: 734KB → 934KB (+200KB、許容範囲)
- [x] README.md の tier2 追加手順に Phase 15 ステップを追記 (taxonomy + traits)
- [x] build_v2_mushrooms.mjs は `--append` モードで既存 traits を保持する設計のため変更不要
- [x] 464/464 unit tests PASS、next build 成功 (127 静的ページ)

#### A1 拡張: taxonomy 7 階層化 (A1 追加対応)

v2.1 ユーザー FB で「目/科/属だけでなく門/亜門/綱/亜綱も表示したい」要望に対応。

- `MushroomTaxonomy` を 7 階層 × `TaxonomyRank { latin, jp }` 構造に拡張 (旧 3 階層は廃止)
- `scripts/phase15/fetch_taxonomy.mjs` / `apply_taxonomy.mjs` で大菌輪個別ページから 112/113 取得
  - phylum / class / order / family / genus は 100% カバレッジ
  - subphylum 100%、subclass 85% (一部菌綱で未定義)
- 詳細ページの学名直下に縦 breadcrumb で表示、rank ラベルピル + 階層色
- Entoloma sarcopus のみ大菌輪未登録で taxonomy 非表示
...
### 15-A 実機 FB 対応 (A7/A8 追加調整)

- A7: 栞バッジ 10px → 13px、20×20 → 22×22 で視認性向上
- A8: 季節タブ種名列 40% (8fr : 12×1fr)、食用分類バッジを iconOnly 記号のみ、リスト並び順を食用分類順に
