...
- **[5] 公開**:
  ...
  - 軽量スキーマ移行（bookmarks 初期化・records の mushroom_id リセット）はアプリ起動時に自動実行
...
**前提**: v2 リリース時点で実利用者は開発者本人のみ、登録数も少数。複雑な ID 救済を行わず、**紐付きを切る割り切り**を採用する。

- `records`（採取記録）: レコード自体は保持。`mushroom_id` フィールドは `null` にリセットし、種名は既存の表示文字列で残す（詳細画面への遷移は再検索で代替）
- `bookmarks`（栞）: **全クリア**。v2 リリース時に初期化する旨をリリースノートで告知
- `plans`（採取計画）内の target species 参照: 種名文字列のみ残し、ID 参照を `null` に

**実装**: 起動時に `schema_version` を確認し、v1 → v2 なら上記を一度だけ実行する軽量スクリプト。詳細マッチングやユーザー通知 UI は作らない（リリースノート告知で十分）。

将来友人配布時は事前バックアップを推奨する旨を設定画面に追記する程度で対応可能。
...
| 移行で紐付きが切れるユーザーデータ | 実利用は開発者本人のみ、登録数少数のため割り切り。records は保持、bookmarks は初期化、リリースノート告知 |
...
- v2 起動時にユーザーデータが破損せず、bookmarks 初期化と records の mushroom_id リセットが一度だけ実行される
...
- **Phase 13-E**: 軽量スキーマ移行（bookmarks 初期化 + records の mushroom_id リセット）
