...
|----|------|------|------|--------|-------------|
| masshurumu | マッシュルーム | Agaricus bisporus | 食用 | `missing` | |
| kinumerigasa | キヌメリガサ | Hygrophorus lucorum | 食用 | `missing` | |
| ootsuga-take | オオツガタケ | Tricholoma zelleri | 食用 | `missing` | |
| shimokoshi | シモコシ | Tricholoma auratum | 食用 | `missing` | |
| ningyoutake | ニンギョウタケ | Albatrellus yasudae | 食用 | `missing` | |
| yanagimatsutake | ヤナギマツタケ | Agrocybe cylindracea | 食用 | `missing` | |
| akahatsu | アカハツ | Lactarius akahatsu | 食用 | `missing` | |
| aishimeji | アイシメジ | Lepista sordida | 食用 | `missing` | |
| chanametsumuratake-modoki | チャナメツムタケモドキ | Pholiota lubrica | 食用 | `missing` | |
| akajikou | アカジコウ | Boletus speciosus | 食用 | `missing` | |
| bunaharitake | ブナハリタケ | Mycoleptodonoides aitchisonii | 食用 | `missing` | |
| numeritsubatake | ヌメリツバタケ | Mucidula mucida | 食用 | `missing` | |
| shakashimeji | シャカシメジ | Lyophyllum fumosum | 食用 | `missing` | |
