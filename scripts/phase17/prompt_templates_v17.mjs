...
6. 数値は資料の値をそのまま引用
7. safety 値は既に指定済、勝手に変更しない（cooking_preservation / poisoning_first_aid / caution の null/非 null 判定のみ安全区分に従う）
8. features は提示された features_raw (統制タグ dict) を散文に置き換えるだけ。新情報の追加禁止`;
...
- mhlw 該当: ${isMhlw ? 'YES (食中毒一次情報源として絶対採用)' : 'no'}
